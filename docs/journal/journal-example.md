...
- **Library evaluation heuristic:** Check GitHub issues _before_ committing. svelte-dnd-action has 40+ open issues but 3k stars. Decided it's mature enough despite some rough edges—better than building from scratch.
...
- **Debugging the duplicate key error:** Error message said "duplicate key at indexes 0 and 1" but didn't explain _why_. Took 2 hours to trace through event handlers and realize both source/destination were adding the same student. Need better mental model of event bubbling.
