<script lang="ts">
  ...
  function handlePointerMove(e: PointerEvent) {
    ...
    pos = {
      ...
      y: clamp(dragStartPos.y + dy, 0, (typeof window !== 'undefined' ? window.innerHeight : 768) - CHIP_H)
    };
  }
  ...
  function onMouseLeave() {
    if (dragging) return;
    collapseTimer = setTimeout(() => { hovering = false; }, 350);
  }
  ...
  $effect(() => {
    function handleKeydown(e: KeyboardEvent) {
      ...
      if (e.altKey && e.shiftKey && e.key.toLowerCase() === 't') {
        e.preventDefault();
        hovering = !hovering;
        if (!hovering) { settingsOpen = false; shareOpen = false; }
        return;
      }
  ...
  // ─── Slide transition ──────────────────────────────────────────────────────
  function slideLeft(_node: Element, { duration = 180 }: { duration?: number } = {}): TransitionConfig {
    return {
      duration,
      easing: cubicOut,
...
{#if expanded}
  ...
  <div
    ...
    class="fixed z-50 flex h-10 select-none items-center rounded-full bg-white shadow-xl ring-1 ring-black/10"
  ...
  >
    ...
    <span class="max-w-[120px] truncate px-1.5 text-sm font-medium text-gray-800">{truncatedName}</span>
    {#if (saveStatus === 'error' || saveStatus === 'failed') && onRetrySave}
      <button type="button" onclick={onRetrySave} class="flex shrink-0 cursor-pointer items-center rounded-full p-1 transition-shadow hover:ring-2 hover:ring-red-300" title={statusTooltip} aria-label={statusTooltip}>
    ...
    <div
      class="flex h-full shrink-0 items-center rounded-r-full pl-1.5 pr-2.5 text-gray-300 hover:text-gray-400 {dragging
        ? 'cursor-grabbing'
        : 'cursor-grab'}"
...
{:else}
  ...
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div
    class="fixed z-50 flex h-10 select-none items-center gap-1.5 rounded-full bg-white pl-2.5 pr-2.5 text-sm font-medium text-gray-700 shadow-xl ring-1 ring-black/10 transition-shadow hover:shadow-2xl {dragging
      ? 'cursor-grabbing'
      : 'cursor-grab'}"
  ...
  >
    ...
    <span class="flex shrink-0 items-center">
      <svg class="h-3 w-2.5 text-gray-300" viewBox="0 0 8 12" fill="currentColor" aria-hidden="true">
        <circle cx="2" cy="2" r="1.1" /><circle cx="6" cy="2" r="1.1" />
        <circle cx="2" cy="6" r="1.1" /><circle cx="6" cy="6" r="1.1" />
        <circle cx="2" cy="10" r="1.1" /><circle cx="6" cy="10" r="1.1" />
      </svg>
